...
sustains_array = {70: correct_tqn * 0.5, 160: correct_tqn * 0.75, 999: correct_tqn * 1.25}  # Sustains length array based on BPM
...
hard_chords_o = {'84, 86, 88': [84, 88], '86, 87, 88': [87, 88], '85, 87, 88': [86, 88], '85, 86, 88': [85, 88], '84, 85, 88': [84, 87], '84, 87, 88': [85, 88], '84, 86, 88': [85, 88],
                 '84, 87, 88': [85, 88], '85, 86, 87': [85, 87], '84, 86, 87': [84, 87], '84, 85, 88': [84, 87], '84, 85, 87': [84, 86], '84, 85, 86': [84, 85]}
hard_chords = {'84, 86, 88': [84, 88], '86, 87, 88': [87, 88], '85, 87, 88': [86, 88], '85, 86, 88': [86, 87], '84, 85, 88': [84, 87], '84, 87, 88': [85, 88], '84, 86, 88': [85, 88],
               '84, 87, 88': [85, 88], '85, 86, 87': [85, 87], '84, 86, 87': [85, 86], '84, 85, 88': [84, 87], '84, 85, 87': [84, 86], '84, 85, 86': [84, 85]}
...
def count_notes(array, start, end, notes, what, instrument):
    ...
    for x in range(0, len(array)):
        ...
        if (((start or end) and start <= array[x][1] <= end) or (start == 0 and end == 0)) and (
                (what == 0 and array[x][2] in notes) or (what == 1 and notes_dict[array[x][2]][1] in notes)):
...
def valid_notes(array, level, first_measure, last_measure, instrumentname, pro, selected):
    ...
    for x in range(0, len(array)):
        ...
        if ((level != '' and notes_dict[note[2]][1] == level) or (level == '' and "notes" in notes_dict[note[2]][1])) and (
            ...
            if skip_bend == 0:
                ...
                if x < len(array_validobjects2) - 1 and uneven == 0:  # If it's the last note, it can't be a bend; no uneven chords on keys can have bends
                    ...
                    if difference <= distance \
                            ...
                            and (
                            sustain >= sustain_length or x == (len(array_validobjects2) - 2) or (array_validobjects2[x + 2][1][0] - array_validobjects2[x + 1][1][0]) >= ((correct_tqn / 2) - how)):
                        # Pitch bend
                        ...
                        array_validobjects.append(
                            [array_validobjects2[x + 1][0], [new_position], array_validobjects2[x + 1][2], array_validobjects2[x + 1][3], [new_length], array_validobjects2[x + 1][5],
                             array_validobjects2[x + 1][6]])
                        skip_bend = 1
                    else:
...
def create_animation_markers(instrument, expression, pause, mute):
    ...
    if len(array_notes) > 0:
        ...
        if play_check == 1 and idle_check == 1 and mute == 0:
            result = RPR_MB("At least one play/mellow/intense and one [idle_realtime] markers are already on the track. Are you sure you want to remove those and create them from scratch?",
                            "Markers found", 1)
        array_temp = []
        if result == 1:
...
def single_snare(instrument, level, what, selected):
    ...
    for x in range(0, len(array_notesevents[0])):
        ...
        if note[2] in notes_dict and (notes_dict[note[2]][1] == leveltext or (note[2] >= 110 and note[2] <= 112)) and (
                selected == 0 or (selected and (this_measure >= first_measure and this_measure <= last_measure))):
            array_validnotes.append(note)
        else:
            array_notes.append(note)
...
def reduce_singlenotes(instrument, level, selected):
    ...
    if (level == 'm' and 'O' in array_chords and (array_chords['O'] * 100) / chord_count <= chord_threshold * 100) or \
            (level == 'e' and 'O' in array_chords and 'B' in array_chords and (array_chords['O'] * 100) / chord_count <= chord_threshold * 100 and (
                    array_chords['B'] * 100) / chord_count <= chord_threshold * 100):
        # Do nothing
        c3 = 0
...
def auto_generalcleanup(instruments, polish, grid, tolerance, invalidmarkers, sustains, whatsustains, odsolo, pedal, rolls):
    PM(instruments)
    PM("\n")
    PM(str(polish) + " - " + str(grid) + " - " + str(tolerance) + " - " + str(invalidmarkers) + " - " + str(sustains) + " - " + str(whatsustains) + " - " + str(odsolo) + " - " + str(
        pedal) + " - " + str(rolls))
...
def auto_vocalscleanup(instruments, compactphrase, trim, grid, tubespace, cleanup, compactharmonies, prec, precgrid, removeinvalid, capitalize, fixtextevents, textv, checkcaps):
    ...
    PM(prec)
    PM("\n")
    PM(str(compactphrase) + " - " + str(trim) + " - " + str(grid) + " - " + str(tubespace) + " - " + str(cleanup) + " - " + str(compactharmonies) + " - " + str(precgrid) + " - " + str(
        removeinvalid) + " - " + str(capitalize) + " - " + str(fixtextevents) + " - " + str(textv) + " - " + str(checkcaps))
    for x in range(0, len(instruments)):
        instrument = instruments[x]
...
def gpimport_drums(instrument, notedata, GPnotes, offset):
    ...
    selection = "E"
    pitches = [["Ride Cymbal (blue)", [99]], ["Crash Cymbal (green)", [100]], ["Hi-Hat (yellow)", [98]], ["Snare (red)", [97]], ["Bass Drum", [96]], ["High Tom (yellow)", [98, 110]],
               ["Mid Tom (blue)", [99, 111]], ["Low/Floor Tom (green)", [100, 112]]]
    velocity = hex(96)
